...
static void initializeFlyoutMenu(buttonState *menu, screenDisplayBuffer *shadowBuf, brogueButton *buttons, windowpos position) {
    ...
    if (rogue.nextGame == NG_FLYOUT_PLAY) {
        ...
        // iOS port (iBrogue): Change Variant / Change Mode removed from the Play
        // flyout. (Variant defaults to Brogue; mode defaults to normal.)
        buttonCount = 2;
        ...
        do {
            if (isApplicationActive()) {
                ...
                if (pauseBrogue(MENU_FLAME_UPDATE_DELAY, (PauseBehavior){.interuptForMouseMove = true})) {
                    ...
                    // iOS port (iBrogue): in-process engine switch. The host can
                    // ask the engine to leave the title so rogueMain() returns and
                    // the engine thread can be torn down. iOS has no Quit button
                    // (can't exit(0)), so this is the clean unwind path.
                    extern volatile boolean brogueCETerminationRequested;
