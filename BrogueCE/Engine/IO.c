...
// large as legibly fits. Coordinates are window cells. Reported just before ceSetExamining.
...
//                  if (playerPathingMap[cursor[0]][cursor[1]] != 1
            ...
            // while it's shown). Host debounces, so a tap-through doesn't flicker. When a
            // box is up, also report its rect (captured by printMonsterDetails/
            // printFloorItemDetails above; printLocationDescription only prints the one-line
            // flavor message, so it doesn't disturb it) so the host can zoom to fit it.
            if (textDisplayed) {
...
signed long applyKeyboardScheme(signed long keystroke, boolean *controlKey, boolean *shiftKey) {
    ...
    switch (rogueKeyboardScheme) {
        case KEYBOARD_SCHEME_MODERN:
            ...
            // cross, ',' as a second down. Real Shift/Ctrl state is already in the modifier flags, so
            // run works via executeKeystroke's controlKey||shiftKey check -- we only translate the
            // character. The vi movement keys h/y/b/n are made inert (see below) so only the grid moves
            // the player; other left-hand commands (apply/drop/search/swap/rest/explore/call +
            // Shift+commands) pass through unchanged.
            switch (keystroke) {
...
void printHelpScreen() {
    ...
    const char modernHelp[BROGUE_HELP_LINE_COUNT][DCOLS*3] = {
        ...
        "    <space/esc>  ****disable keyboard cursor control",
        "",
        "  a/r/t/d/c/R/w  ****apply/remove/throw/drop/call/relabel/swap an item",
