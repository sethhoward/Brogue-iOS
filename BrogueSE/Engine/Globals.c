...
// iOS port (Brogue SE): the cast sunlight gently flickers now (small rand + colorDances) instead of a
// dead-static glow, so a sunlit room feels alive as if lit from outside. Same warm base as before. Reinforces
// the dappled sunlit-floor colors below. Cosmetic variance -> no game-state/save impact.
const color sunLightColor =         {100,   100,    75,     5,      5,          8,          0,      true};
// iOS port (Brogue SE): dappled-sunlight FLOOR colors for SUNLIGHT_POOL, which previously used the flat static
// floor colors (hence the dead-looking bright patch). Warm and DANCING (colorDances + rand): shuffleTerrainColors
// shimmers each sunlit cell independently every idle tick, so the light plays across the floor like sun filtered
// through vegetation outside. Cosmetic (RNG_COSMETIC via shuffleTerrainColors) -> deterministic/replay-safe.
const color sunlitFloorForeColor =  {40,    40,     28,     12,     12,         4,          15,     true};
const color sunlitFloorBackColor =  {16,    14,     7,      10,     9,          0,          0,      true};
...
const floorTileType tileCatalog[NUMBER_TILETYPES] = {
 ...
 /*TORCH_WALL*/                 {G_TORCH,       &torchColor,        &wallBackColor,     0,  0,  DF_PLAIN_FIRE,0,0,                          0,  TORCH_LIGHT,    (T_OBSTRUCTS_EVERYTHING), (TM_STAND_IN_TILE | TM_LIGHT_BREATHES),                                    "a wall-mounted torch", "The torch is anchored firmly to the wall and sputters quietly in the gloom."}, // iOS port (Brogue SE): TM_LIGHT_BREATHES -- torchlight gently breathes
 ...
 /*SUNLIGHT_POOL*/              {G_FLOOR,   &sunlitFloorForeColor,  &sunlitFloorBackColor,90,0,DF_PLAIN_FIRE,0,0,                                              0,  SUN_LIGHT,  (0), (TM_STAND_IN_TILE),                                                            "a patch of sunlight",  "sunlight streams through cracks in the ceiling."}, // iOS port (Brogue SE): dappled dancing colors (was flat floorFore/BackColor)
 ...
 // iOS port (Brogue SE): #812 — forcefield/melt carry T_OBSTRUCTS_SURFACE_EFFECTS like every other
 // solid obstruction (CRYSTAL_WALL, statues, walls). Without it, surface-layer effects spread through
 // the staff-of-obstruction barrier: notably a GAS_EXPLOSION (e.g. an explosive mutation) flood-fills
 // across a "sealed" region, since spawnMapDF only stops surface spread at T_OBSTRUCTS_SURFACE_EFFECTS.
 /*FORCEFIELD*/                 {G_CRYSTAL, &forceFieldColor,       &forceFieldColor,   0,  0,  0,0,DF_FORCEFIELD_MELT,                  -200,  FORCEFIELD_LIGHT, (T_OBSTRUCTS_PASSABILITY | T_OBSTRUCTS_GAS | T_OBSTRUCTS_SURFACE_EFFECTS | T_OBSTRUCTS_DIAGONAL_MOVEMENT), (TM_STAND_IN_TILE | TM_VANISHES_UPON_PROMOTION | TM_PROMOTES_ON_CREATURE),       "a green crystal",      "The translucent green crystal is melting away in front of your eyes."},
 /*FORCEFIELD_MELT*/            {G_CRYSTAL, &black,                 &forceFieldColor,   0,  0,  0,0,0,                                 -10000,  FORCEFIELD_LIGHT, (T_OBSTRUCTS_PASSABILITY | T_OBSTRUCTS_GAS | T_OBSTRUCTS_SURFACE_EFFECTS | T_OBSTRUCTS_DIAGONAL_MOVEMENT), (TM_STAND_IN_TILE | TM_VANISHES_UPON_PROMOTION | TM_PROMOTES_ON_CREATURE),     "a dissolving crystal",     "The translucent green crystal is melting away in front of your eyes."},
